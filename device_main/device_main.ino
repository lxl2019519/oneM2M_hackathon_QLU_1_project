...
void loop() {
    ...
    //If the current grain weight is insufficient
	if (value_weight_c < 10.00 ) 
   {
       digitalWrite(ledPinUlt, HIGH);
       Serial.println("The red LED has lit up to warn that there is not enough food");
       delay(10);
       openServo(); delay(600); closeServo();delay(600);
       openServo();delay(600);closeServo();delay(600);
       openServo();delay(600);closeServo();delay(600);
       closeeServo();
       Serial.println("The servo is already turning");
     }
     if (value_weight_c > 10.00 )
     {  
    ...
    while (!client.available()) { //Wait until if there is data to read
        delay(1);
    }
...
//Sending ultrasonic measured data
void pushUltrasound() {
    ...
    Serial.println("-------- Value of ultrasonic distance measurement ----------");
...
//Sending the data measured by the load cell
float pushWeight() {
...
// Water level sensor mapped to A0
void pushWaterHigh() {
    double temp;
...
//Sending the data measured by the temperature and humidity senso
void pushTemperature() {
	...
	Serial.println("-------------- the value of tem & hum ------------");
	Tem=(int)temperature;
	Hum=(int)humidity;
